...
For simple algorithms, you may be able to construct a formula determining how many instructions it will take to complete for a given input (for non-simple algorithms, let's save the Halting Problem for another day). Discard all but the largest term of sums for that formula and you have the [Big-O](https://en.wikipedia.org/wiki/Big_O_notation) time complexity of the algorithm. For an input of _n_ items, a linear complexity, _O(n)_, is ideal, _O(log2(n))_ even better. If you know the input is manageably small, _O(n.log2(n))_ or possibly even _O(n2)_, might be acceptable. Anything exponential e.g. _O(2n)_, should be a red flag that [another approach is needed](/follies/helping). Premature optimisation and trying to squeeze clock cycle efficiency out of code is unnecessary in almost all cases, but this low bar of algorithmic efficiency isn't theoretical naval gazing but necessary to create responsive products that respect users' time.
