...
For simple algorithms, you may be able to construct a formula determining how many instructions it will take to complete for a given input (for non-simple algorithms, let's save the Halting Problem for another day). Discard all but the largest term of sums for that formula and you have the [Big-O](https://en.wikipedia.org/wiki/Big_O_notation) time complexity of the algorithm. For an input of $n$ items, a linear complexity, $O(n)$, is ideal, $O(\log_{2}(n))$ even better. If you know the input is manageably small, $O(n.\log_{2}(n))$ or possibly even $O(n^{2})$, might be acceptable. Anything exponential e.g. $O(2^{n})$, should be a red flag that [another approach is needed](/follies/helping). Premature optimisation and trying to squeeze clock cycle efficiency out of code is unnecessary in almost all cases, but this low bar of algorithmic efficiency isn't theoretical naval gazing but necessary to create responsive products that respect users' time.
